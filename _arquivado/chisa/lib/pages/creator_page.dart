...
import 'package:chisa/anki/anki_field_mapping_dialog.dart';
...
class CreatorPageState extends State<CreatorPage> {
  ...
  /// Load available Anki models and the user's saved field mapping.
  Future<void> _loadModels() async {
    try {
      _availableModels = await getModels();
      _selectedModelName = appModel.sharedPreferences
          .getString('anki_last_model_name');
      if (_selectedModelName != null &&
          _availableModels.containsKey(_selectedModelName)) {
        _modelFieldNames = _availableModels[_selectedModelName]!;
        // Restore saved field mapping
        final savedMapping = appModel.sharedPreferences
            .getString('anki_field_mapping_$_selectedModelName');
        if (savedMapping != null) {
          _fieldMapping = Map<String, String>.from(
            (savedMapping.isNotEmpty) ? Map<String, String>.from(
              Uri.splitQueryString(savedMapping).map(
                    (k, v) => MapEntry(k, v),
                  ),
            ) : {},
          );
      ...
      // Models not available (Anki not open / AnkiDroid not installed)
    }
  }

  /// Dropdown to select the Anki note type (model).
  Widget _buildModelSelector() {
    if (!_modelsLoaded || _availableModels.isEmpty) {
      return const SizedBox.shrink();
    }

    final modelNames = ["Default (jidoujisho Chisa)", ..._availableModels.keys];
        ...
        children: [
          ...
          Expanded(
            child: DropdownButtonFormField<String>(
              value: modelNames.contains(_selectedModelName)
                  ? _selectedModelName
                  : modelNames.first,
              isExpanded: true,
              isDense: true,
              ...
              items: modelNames.map((name) {
                final isDefault = name.startsWith("Default");
                final displayName =
                    isDefault ? "Default (jidoujisho Chisa)" : name;
              ...
              onChanged: (val) {
                setState(() {
                  ...
                  _modelFieldNames =
                      val != null ? (_availableModels[val] ?? []) : [];
                  _fieldMapping = null; // reset mapping when model changes
                  appModel.sharedPreferences.setString(
                      'anki_last_model_name', val ?? "");
                  appModel.sharedPreferences.setString(
                      'anki_field_mapping_$val', "");
                });
              },
            ),
          ),
          if (_modelFieldNames.isNotEmpty) ...[
            const SizedBox(width: 4),
            _buildFieldMappingButton(),
          ],
        ],
      ),
    );
  ...
  /// Button that opens the field mapping dialog.
  Widget _buildFieldMappingButton() {
    return IconButton(
      ...
      onPressed: () async {
        final result = await showDialog<Map<String, String>>(
          context: context,
          builder: (ctx) => AnkiFieldMappingDialog(
            ankiFieldNames: _modelFieldNames,
            currentMapping: _fieldMapping,
          ),
        );
        if (result != null && _selectedModelName != null) {
          setState(() {
            _fieldMapping = result;
            // Persist mapping
            final queryString = Uri(queryParameters: result).query;
            appModel.sharedPreferences.setString(
                'anki_field_mapping_$_selectedModelName', queryString);
          });
        }
      },
    );
