...
/// Controls a DLNA/UPnP cast session directly (no Jellyfin server mediation).
///
/// Sends the stream URL to a TV/renderer via UPnP AVTransport,
...
class DlnaController extends CastSession {
    ...
    try {
      ...
      for (final svc in services) {
        ...
        if (serviceType.toLowerCase().contains('avtransport')) {
          final relative = svc.getElementsByTagName('controlurl').firstOrNull?.text ?? '';
  ...
  Future<void> play() async {
    final body = _buildSoapEnvelope('Play', '<InstanceID>0</InstanceID><Speed>1</Speed>');
    try {
      await http.post(Uri.parse(_controlUrl), headers: _soapHeaders('Play'), body: body);
  ...
  Future<void> pause() async {
    ...
    try {
      await http.post(Uri.parse(_controlUrl), headers: _soapHeaders('Pause'), body: body);
  ...
  Future<void> stop() async {
    ...
    try {
      await http.post(Uri.parse(_controlUrl), headers: _soapHeaders('Stop'), body: body);
    } catch (_) {}
    _state = CastSessionState.ended;
    notifyListeners();
  }
  ...
  Future<void> seek(Duration position) async {
    ...
    try {
      await http.post(Uri.parse(_controlUrl), headers: _soapHeaders('Seek'), body: body);
      _position = CastPosition(
